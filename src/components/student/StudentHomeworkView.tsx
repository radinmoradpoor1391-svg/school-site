...
import { uploadHomeworkAttachment } from '../../firebase/storage';
...
export const StudentHomeworkView: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent, hwId: string) => {
    ...
    try {
      ...
      let downloadUrl = imagePreview || undefined;
      if (selectedFile) {
        try {
          downloadUrl = await uploadHomeworkAttachment(hwId, currentStudent.id, selectedFile);
        } catch (err: any) {
          console.warn('Fallback to base64 for homework upload:', err);
        }
      }
